(function($) {
    ...
    $(function() {
        ...
        var createRowFromUploadItem = function(ui) {
            ...
            var $progressBar = $('<div/>').addClass('progress-bar recepcion').css('width', '0%');
            ...
            var $cancelBtn = $('<a/>').attr('href', 'javascript:').append(
                $('<span/>').addClass('glyphicon glyphicon-remove negro')
            ).on('click', function() {
            ...
            if (previewsOn) {
                ...
                $preview = $('<i>No disponible</i>');
            }
           
            totalbytes=totalbytes+(ui.file.size*1.11);
            // Appending cells to row
            $('<td/>').append($preview).appendTo($row); // Preview
            $progressBar['filename']=$('<td/>').text(ui.file.name).appendTo($row); // Filename
        ...
        var fileAddHandler = function(e) {
            ...
            ui.completeCallback = function(success, data, errorCode) {
               ...
               // log((this.file.name || "[custom-data]") + " completed");
                if (success) {
                    ...
                    if(prueba.files[0]["error"]=='El XML se ha guardado correctamente'){

                       classs='text-success';

                    }

                    $progressBar["filename"].html($progressBar["filename"].text()+'<p><b class="'+classs+'">'+prueba.files[0]["error"]+'</b></p>');
            ...
            ui.progressCallback = function(percent,loaded) {
                $progressBar.css('width', Math.round(percent) + '%');

                totalcargado=totalcargado+loaded;
                progreso = ((totalcargado * 100) / totalbytes);

                $('#total_progreso').css('width', progreso+'%');
            };
...
if(!$.support.fileSelecting) {
    ...
    if(!$.support.fileReading) {
        //("[-] Your browser doesn't support file reading on client side");
    }
